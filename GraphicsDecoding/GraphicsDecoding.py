...
# PYTHON ROUTINE zur Übertragung von grafischen Strukturen (Darstellung durch RBG-Werte) durch ein GAN-basiertes künstliches neuronales Netzwerk #
# Der RGB-Code ist durch 3 x 8 Bits dargestellt. Im File ist nur ein RGB implemetiert. Der Source Code ist auf höhere Dimnesionen für unterschiedliche RGB-Werte anzupassen.
...
# Definition von Funktionen zur Verschlüsselung a kongruent b mod 52 durch GAN - hier nur Werte von 0 bis 9

def letters_to_value(x): # Definiere eine Funktion zur Abbildung von Strings auf Zahlen a kongruent b mod 52 - hier nur Werte von 0 bis 9
...
def value_to_letters(x): # Definiere eine Funktion zur Abbildung von Zahlen auf Strings a kongruent b mod 52 - hier nur Werte von 0 bis 9
...
with open("RGBCodeDeclaration.txt") as file: # Importiere den Zeichenstring aus dem externen File mit Bezeichnung CodeDeclaration.txt
...
for j in range(0,len(S)-1): # Entschlüsselung von Strukturen durch den übertragenen Zahlencode

    if (fS[j] != -1): fSS = fSS + value_to_letters((fS[j] - dkey[j]) % len(alphabet)) # Entschlüsselung im String
    else: fSS = fSS + ' '

print(' ')
print('Originaler RGB-Dezimalcode: ', S)
print(' ')
print('Verschlüsselter RGB-Dezimalcode: ', fS)
print('Entschlüsselter RGB-Dezimalcode: ',fSS)
print(' ')
print('Generierter RGB-Dezimal-Schlüssel: ', dkey)
print(' ')
